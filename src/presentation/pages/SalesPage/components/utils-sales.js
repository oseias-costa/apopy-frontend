export const variablesSales = (stockState, newSale) => {
...
export const variablesReverseSales = (stockState) => {
    const variables = {
        _id: stockState._id,
        category: stockState.category,
        costPrice: stockState.costPrice,
...
export const originStockMoviment = (stockState, newSale) => {
    const originStock = {
        _id: stockState._id,
        category: stockState.category,
        product: stockState.product,
...
export const originStockReverse = (saleState) => {
    const originStock = {
...
export const uniqueDataSales = (sale) => {
    var _a;
    let category = [];
    let product = [];
    let suplier = [];
    ...
    for (let saleItem of sale) {
        ...
        const yearConverted = (_a = new Date(saleItem.date).getFullYear()) === null || _a === void 0 ? void 0 : _a.toString();
...
export const filterSales = (sale, filteredSale) => {
    var _a;
    let list = [];
    for (let i = 0; i < sale.length; i++) {
        const monthConverted = convertMonth(new Date(sale[i].date).getMonth());
        const yearConverted = (_a = new Date(sale[i].date).getFullYear()) === null || _a === void 0 ? void 0 : _a.toString();
...
const convertMonth = (month) => {
    ...
    for (let i = 0; i < months.length; i++) {
        if (months[i].monthNumber === month) {
            result = (_a = months[i].month) === null || _a === void 0 ? void 0 : _a.toString();
        }
    }
    return result;
};
